...
<div style={{ maxWidth: "80%", fontSize: '.75em'}}>
  <ul>
    ...
    <li style={{marginBottom: '15px'}}>You've been following along</li>
...
```diff 4,5,6,7,8 title="Because the domain is finite, we can precompute all the expected inputs/outputs"
```

```diff 4,5,6,7,8 title="This is trivial. If you have your inputs/outputs you can just use a lookup"
```

```diff 10,11 title="Unless you want to guard against bad input"
```
...
const {output, logOutput} = addOne(1, console)
output // 2
